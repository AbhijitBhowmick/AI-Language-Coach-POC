...
public interface QuestionEntityRepository extends JpaRepository<QuestionEntity, UUID> {
    ...
    @Query("SELECT q FROM QuestionEntity q WHERE q.targetLanguage = :lang AND q.targetLevel = :level AND q.active = true")
    List<QuestionEntity> findByTargetLanguageAndTargetLevelAndActiveTrue(String lang, String level);
